...
resource "aws_ecs_task_definition" "ecs_task_workshop" {
  ...
  # Usamos una imagen inicial para el bootstrap
...
resource "aws_ecs_service" "ecs_service_workshop" {
  ...
  # Terraform crea el servicio la primera vez, pero luego ignora los cambios 
  # en la imagen para que GitHub Actions tome el control.
  lifecycle {
